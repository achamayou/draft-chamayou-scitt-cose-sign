...
a specific set or threshold of distinct Issuers or distinct combinations of
Issuer and Subject.  `COSE_Sign1` verification and its body `iss` and `sub`
...
requirement independently of the signatures array.  It MUST count distinct,
successfully verified, and authorized Issuers after applying its identity and
alias rules, rather than counting signatures, keys, or merely unequal `iss`
strings.  One authority can control multiple identifiers or keys.  This
document does not provide atomic "all issuers or none" semantics before
Registration.
